function [edfStruct1, datestr, gx, gy, gstx, gsty, genx,...
    geny,pupil_a, st_times, en_times, tmp_dvnt, tmp_resp, tmp_side, tmp_q, tmp_rt] = ...
    visualadapt_dataimp_main(path, eye, typ)
...
%------- test orientation-------
tmp_dvnt = psych_dat.orientation;% Selected deviant
tmp_dvnt = tmp_dvnt(~arrayfun( @isnan, tmp_dvnt)); % Remove the empty character cell

% Enter true deviant values


% ------------- Key response ------------
tmp_resp = psych_dat.key_resp_7keys; % Key response
tmp_resp = tmp_resp(~cellfun( @isempty, tmp_resp)); % Remove the empty character cell
tmp_resp = cellfun(@(x) ...
    (strcmp(x, '[''right'']') * 1) + ...   % 'right' -> 1
    (strcmp(x, '[''left'']') * 0) + ...
    (strcmp(x, 'None') * -1), tmp_resp); %code key response

%----------- Deviant side (For experiment 2) --------
if strcmpi(typ, 'nwm2') | strcmpi(typ, 'wm2') % Store deviant side
    tmp_side = psych_dat.grating; % Key response
    tmp_side = tmp_side(~cellfun( @isempty, tmp_side)); % Remove the empty character cell
    tmp_side = cellfun(@(x) ...
    (strcmp(x, 'grating_10') * 1) + ...   % 'right' -> 1
    (strcmp(x, 'grating_11') * 0) ...
    , tmp_side); %code key response

    
else
    tmp_side = nan(size(tmp_dvnt));
...
%----------- Reaction time-----------------
tmp_rt = psych_dat.key_resp_7rt; % Adaptor train
% tmp_rt = cell2mat(cellfun(@str2num, tmp_rt, 'UniformOutput', false));
for i = 1:length(tmp_rt)
    if isempty(tmp_rt{i})  % Check if the current cell is empty
        tmp_rt{i} = NaN;  % Replace with empty array
    else
        numStr = regexp(tmp_rt{i}, '\[(.*?)\]', 'tokens');
        tmp_rt{i} = str2double(numStr{1}{1});  % Convert string to double
    end
end
